    ...
    def _set_label(
        ...
        def get_font_size(font_size, label_type, text_on_label, barcode_module_size):
            if font_size != -1:
                ...
                font_size = 1
                barcode_image = LabelComponentBarcodeCode128(
                    text_on_label, barcode_module_size, quiet_zone_size_factor=0
                ).get_image()
        ...
        def get_barcode_module_size(
            barcode_module_size, label_type, font_size, text_on_label
        ):
            if barcode_module_size != -1:
                ...
                if label_type == 1 or label_type == 2:
                    while True:
                        barcode_image = LabelComponentBarcodeCode128(
                            text_on_label, module_size, quiet_zone_size_factor=0
                        ).get_image()
        ...
        def calculate_label_dimensions(
            component_barcode, component_text, separator_line_thickness, label_type
        ):
            """"""
            dimensions = (-1, -1)
            if label_type == 0 or label_type == 1:
                ...
                height = (
                    ...
                    + component_barcode.get_image().size[1]
                    + component_text.get_image().size[1]
                )
                dimensions = (width, height)
            elif label_type == 3:
                width = (
                    3 * separator_line_thickness
                    + component_barcode.get_image().size[0]
                    + component_text.get_image().size[0]
                )
                height = (
                    2 * separator_line_thickness + component_barcode.get_image().size[1]
                )
        ...
        def assemble_components(
            ...
            def draw_separator_lines(
                label_image, component_barcode, separator_line_thickness, label_type
            ):
                result = ImageDraw.Draw(label_image)
                for i in range(separator_line_thickness):
                    result.line([(0, i), (label_dimensions[0] - 1, i)], fill="black")
                    result.line(
                        [
                            (0, label_dimensions[1] - 1 - i),
                            (label_dimensions[0] - 1, label_dimensions[1] - 1 - i),
                        ],
                        fill="black",
                    )
                    result.line([(i, 0), (i, label_dimensions[1] - 1)], fill="black")
                    result.line(
                        [
                            (label_dimensions[0] - 1 - i, 0),
            ...
            if label_type == 0:
                ...
                result.paste(
                    component_text.get_image(),
                    (
                        separator_line_thickness
                        + round(
                            label_dimensions[0] / 2
                            - component_text.get_image().size[0] / 2
                        ),
                        round(
                            label_dimensions[1] / 2
                            - component_text.get_image().size[1] / 2
                        ),
                    ),
        ...
        def draw_corner_trim_lines(label_image, line_width, barcode_padding_thickness):
            ...
            offset = round(
                (
                    math.sqrt(size * math.pow(barcode_padding_thickness, 2))
        ...
        self.size_barcode_module = get_barcode_module_size(
            size_barcode_module, label_type, size_font, self.text_on_label
        )
        self.size_font = get_font_size(
            size_font, label_type, self.text_on_label, self.size_barcode_module
        )
        ...
        self.component_barcode = LabelComponentBarcodeCode128(
            self.text_on_label,
            self.size_barcode_module,
            height=self.component_text.get_image().size[1],
        )
        ...
        self.label_image = draw_corner_trim_lines(
            self.label_image, separator_line_thickness, 2 * self.size_barcode_module
        )
