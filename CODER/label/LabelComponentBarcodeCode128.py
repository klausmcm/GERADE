...
class LabelComponentBarcodeCode128:
    def __init__(self, text, module_size, height=1, quiet_zone_size_factor=10):
        ...
        def _add_quiet_zones(barcode_image, module_size, size_factor):
            if (
                isinstance(module_size * size_factor, int)
                and module_size * size_factor > 0
            ):
        ...
        self.barcode_image = _add_quiet_zones(
            _resize_barcode(
                _trim_barcode(
                    _generate_barcode(self.text_encoded, module_size), module_size
                ),
                round(height),
            ),
            module_size,
            quiet_zone_size_factor,
        )
