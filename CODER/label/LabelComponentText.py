            ...
            if font_size <= 0:
                ...
                if file_path_true_type_font != "":
                    ...
                    label_font = ImageFont.truetype(
                        "/media/sf_shared/workspace/barcode/files/DejaVuSansMono.ttf",
                        font_size,
                    )
                image_width = ImageDraw.Draw(
                    Image.new("RGB", (0, 0), "white")
                ).textsize(text, font=label_font)[0]
                image_height = ImageDraw.Draw(
                    Image.new("RGB", (0, 0), "white")
                ).textsize(string.ascii_letters + string.digits, font=label_font)[1]

                lines = text.split("\n")

                img = Image.new("RGBA", (image_width, len(lines) * image_height))
                for i in range(len(lines)):
                    image_line = Image.new("RGBA", (image_width, image_height))
                    draw = ImageDraw.Draw(image_line)
                    draw.text((0, 0), lines[i], fill=color, font=label_font)
