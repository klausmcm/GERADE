...
class LabelComponentBarcodeDataMatrix:
    def __init__(self, text, module_size, quiet_zone_thickness=-1):
        ...
        def _trim_barcode(barcode_image):
            ...
            cropped_image = barcode_image.crop(
                (
                    2 * self.module_size,
                    2 * self.module_size,
        ...
        def _add_quiet_zone(barcode_image):
            """"""
            # TODO: document
            barcode_with_whitespace = Image.new(
                "RGB",
                (
            ...
            barcode_with_whitespace.paste(
                barcode_image, (self.quiet_zone_thickness, self.quiet_zone_thickness)
            )
            return barcode_with_whitespace

        self.quiet_zone_thickness = (
            2 * module_size if quiet_zone_thickness == -1 else quiet_zone_thickness
        )
        self.text_encoded = "".join(
            [c for c in text if c in string.ascii_letters + string.digits]
        )
        self.module_size = module_size
        self.barcode_image = _add_quiet_zone(
            _trim_barcode(_generate_barcode(self.text_encoded, module_size))
        )
